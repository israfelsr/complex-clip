...
def _default_collate(batch):
    ...
    # For curious reader, this is what we added to the original code:
    elif isinstance(elem, PIL.Image.Image) or isinstance(
        elem, PIL.JpegImagePlugin.JpegImageFile
    ):
        return batch
    elif isinstance(elem, collections.abc.Mapping):
        try:
            return elem_type(
                {key: _default_collate([d[key] for d in batch]) for key in elem}
            )
        except TypeError:
        ...
        if isinstance(elem, tuple):
            return [
                _default_collate(samples) for samples in transposed
